...
export default class IOSSDKManager extends cc.Component {
  ...
  public videoCallbackHandler (backString: string) {
    ...
    const temp: string[] = backString.split('_')
    const code: number = Number.parseInt(temp[0])
    const type: number = Number.parseInt(temp[1])
  ...
  public isReadyAd (adType: AdType): boolean {
    ...
    if (pid === '') return false
    const isReady = this.isAdReady(adType.toString(), pid) + '' === 'true'
    return isReady
  }
  ...
  shareToWx (title: string = null, toScene: ShareType, callback = null) {
    ...
    jsb.reflection.callStaticMethod('YZK', 'shareApp:description:scene:imageUrl:appId:', title || this.shareTitle, this.shareContent, toScene, this.shareSpriteUrl, this.appStoreId)
  ...
  onShareReturn () {
    ...
    const delta = Date.now() - this.outTime
