...
export class LargeNumber {
  ...
  toFormatString () {
    ...
    const inedx = Math.floor((str.length - 1) / 3)
  ...
  private static numberToArray (numStr: string) {
    ...
    numStr = numStr.trim()
    const temp: number[] = new Array<number>(LargeNumberLength)
    temp.fill(0)
  ...
  mul (num: number | LargeNumber | string) {
    const temp: LargeNumber = new LargeNumber(0)
    ...
    for (let i = 0; i < str_num.length; i++) {
      ...
      for (let j = 0; j < this.value.length; j++) {
        ...
        const bit = this.value.length - j - 1 + str_num.length - i - 1
