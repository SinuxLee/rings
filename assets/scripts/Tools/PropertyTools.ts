...
cc.Node.prototype.addPosX = function (x: number, posType: PositionType) {
  if (posType == PositionType.local) {
    const localPosition: cc.Vec2 = this.position
    localPosition.x += x
    this.position = localPosition
  } else {
    const position = this.parent.convertToWorldSpaceAR(this.position)
    position.x += x
...
cc.Node.prototype.getWorldP = function (): cc.Vec2 {
  if (!this.parent) {
    ...
    const pos = this.parent.convertToWorldSpaceAR(this.position)
...
cc.Node.prototype.moveTo = function (duration: number, x: number, y: number, type: PositionType): cc.ActionInterval {
  if (type === PositionType.local || this.parent == null) {
    ...
    const worldP = cc.v2(x, y)
    const localP = this.parent.convertToNodeSpaceAR(worldP)
...
cc.Node.prototype.moveBy = function (duration: number, x: number, y: number, type: PositionType): cc.ActionInterval {
  if (type === PositionType.local || this.parent == null) {
    ...
    const worldP = this.getWorldP().addSelf(cc.v2(x, y))
    const localP = this.parent.convertToNodeSpaceAR(worldP)
...
Array.prototype.GetRandomIndexList = function (count: number): number[] {
  const targetList: number[] = new Array<number>()
  const indexList: number[] = new Array<number>()
  ...
  while (targetList.length < count) {
    const a = Random.RangeInt(0, indexList.length)
...
/* String************************************ */
String.prototype.ToNumberArray = function (separator: string): number[] {
  const str: string[] = this.split(separator)
...
Number.prototype.degreesToVectors = function (): cc.Vec2 {
  const radian = cc.misc.degreesToRadians(this) // 将角度转换为弧度
  const comVec = cc.Vec2.RIGHT // 一个水平向右的对比向量
  const dirVec = comVec.rotate(radian) // 将对比向量旋转给定的弧度返回一个新的向量
  return dirVec
}

/* cc.Vec2  ******************************************** */
cc.Vec2.prototype.toAngle = function (): number {
  const radian = -cc.Vec2.UP.signAngle(this)
  return cc.misc.radiansToDegrees(radian) // 将弧度转换为角度
}

cc.Vec2.prototype.getAngle = function (dir: cc.Vec2): number {
  const radian = -this.signAngle(dir)
  return cc.misc.radiansToDegrees(radian) // 将弧度转换为角度
}

/* cc.Vec3  ******************************************** */
cc.Vec3.prototype.toAngle = function (): number {
  const radian = -cc.Vec2.UP.signAngle(cc.v2(this))
