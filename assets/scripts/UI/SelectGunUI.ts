...
export default class SelectGunUI extends UIPanelBase {
  ...
  private readonly itemList: SelectGunItem[] = []
  ...
  initUI () {
    const length = GunDataManager.getInstance().getGunCount()
    for (let i = 0; i < length; i++) {
      const node = cc.instantiate(this.itemPrefab)
      node.setParent(this.itemParent)
      const item = node.getComponent(SelectGunItem)
  ...
  updateUI () {
    const ownGuns = GameManager.getInstance().playerData.ownGuns
  ...
  btnOfSelectItem (e, index) {
    ...
    const ownGuns = GameManager.getInstance().playerData.ownGuns
    if (ownGuns[index] != 1) {
      const high = GunDataManager.getInstance().getGunData(index).high
      const condition = `达到${high}解锁`
